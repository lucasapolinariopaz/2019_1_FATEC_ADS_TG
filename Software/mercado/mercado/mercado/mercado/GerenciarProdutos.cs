...
namespace mercado
{
    public partial class GerenciarProdutos : Form
    {
        ...
        private void carregarCombobox(object sender, EventArgs e)
        {
            ...
            cb_cat.Items.Add("");
            string consulta_sql = "SELECT e.categoria_prod FROM estoque e GROUP BY e.categoria_prod;";
            SqlConnection conn = conexao.obterConexao();
            SqlCommand commn = new SqlCommand(consulta_sql, conn);
            commn.CommandType = CommandType.Text;
            commn.Parameters.Add(new SqlParameter("@categoria_prod", "categoria_prod"));
            ...
            if (result == true)
            {
                while (dr.Read())
                {
                    cb_cat.Items.Add(dr["categoria_prod"].ToString());
                }
            }
            else
            {
            ...
            cb_marca.Items.Add("");
            consulta_sql = "SELECT e.marca_prod FROM estoque e GROUP BY e.marca_prod;";
            ...
            if (result == true)
            {
                while (dr.Read())
                {
                    cb_marca.Items.Add(dr["marca_prod"].ToString());
                }
            }
            else
            {
